...
/*java -cp .;commons-io-2.8.0.jar 工具.生成文件记录.生成本地文件记录 */
/**
 * 用以输出本地文件记录
 * @author aotmd
 * @version 1.0.4
...
public class 生成本地文件记录 {
    ...
    public static void main (String[]args)  {
        ...
        BufferedWriter bw;
        try {
            bw=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(index.getPath()+File.separator+"文件记录.txt"),"UTF-8"));
    ...
    public void cycle(File file, int count, char[] empty) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (temp.isDirectory()) {
                ...
                /*输出名称,大小*/
                sb.append(String.format("%s \t [%s]", temp.getName(), this.getPathSize(temp))).append('\n');
            }
            /*如果是最后一个*/
            if (i == list.size() - 1) {
